...
class DiceGameStatisticsAnalyse:
    def __init__(self, nb_dice, nb_turn, interval):
    ...
    def __str__(self):
        output_str = 'Max score: ' + str(self._max_score) + '\n'
        output_str += 'Mean score: ' + str(self._mean_score) + '\n'
        output_str += 'Max roll in turn: ' + str(self._max_nb_roll) + '\n'
        output_str += 'Max bonus in turn: ' + str(self._max_bonus) + '\n'
        output_str += 'Number of dices: ' + str(self._nb_dice)
    ...
    def launch(self):
        ...
        while turn_index < self._nb_turn:
            ...
            if turn_score > self._max_score:
                self._max_score = turn_score
        ...
        self._mean_score /= self._nb_turn
...
class DiceGameDistributionAnalyse:
    def __init__(self, nb_turn, interval, nb_dice):
        self._nb_dice = nb_dice
        self._nb_turn = nb_turn
        self._interval = interval
...
class ExcelStatsGenerator:
    ...
    def export_excel(self):
        ...
        df = panda.DataFrame(statistics, columns=['', 'Max', 'Mean'])

        df.to_excel(r'D:\Hetic\export_dice_game_stats.xlsx', index=False, header=True)
...
dice_distribution_statistics = DiceGameDistributionAnalyse(1000, 50, 5)
